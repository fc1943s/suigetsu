...
module Client =
    ...
    let inline listen<'UIState, 'SharedServerMessage, 'SharedClientMessage>
        ...
        let init () =
            let state =
                { Connection = Disconnected
                  User = { InternalUI.Id = string (System.Random().Next ()) }
                  Users = []
            ...
            let newState, cmd =
                ...
                | InternalServerMessage msg ->
                    ...
                    | InternalUI.WelcomeUser (users, sharedState) ->
                        { state with
                              Users = users
                              UIState = sharedState }, Some (InternalUI.InternalClientMessage<'SharedClientMessage>.Connect state.User)

                    | InternalUI.SetState newState ->
                        { state with UIState = newState }, None

                    | InternalUI.AddUser user ->
                        (if user.Id = state.User.Id then { state with Connection = Connected }
                         else { state with Users = user :: state.Users }), None

                    | InternalUI.RemoveUser user ->
                        { state with Users = state.Users |> List.filter (fun { Id = id } -> id <> user.Id) }, None

                    | InternalUI.SharedServerMessage msg ->
                        let (sharedState, cmd) = handler msg state.UIState
                        
                        let cmd = cmd |> Option.map InternalUI.InternalClientMessage<'SharedClientMessage>.SharedClientMessage
        ...
        let view =
            ...
            let mainComponentFactory
                (state: PrivateState<'UIState>)
                (serverToClientDispatch: PrivateServerMessage<'SharedServerMessage, 'UIState> -> unit) =
