...
module Logging =
    ...
    type private DelegateLogEventSink () =
        ...
        interface ILogEventSink with
            member _.Emit logEvent =
                let emit () = _sinks |> Seq.iter (fun x -> x.Emit logEvent)
                
                if Testing.isTestingLazyIo () || logEvent.Level >= LogEventLevel.Warning
                then emit ()
                else async { emit () } |> Async.Start
                
        static member AddSink (sink: ILogEventSink) =
            _sinks.Add sink
    ...
    type private LogFilter (filters: FilterList, __excludingFilters: unit) =
        ...
        interface ILogEventFilter with
            member _.IsEnabled logEvent =
                let property = logEvent.Properties.TryGetValue PROP_SOURCE_CONTEXT |> snd
                match property with
        ...
        interface ILogEventEnricher with
            member _.Enrich (logEvent, propertyFactory) =
                if not (logEvent.Properties.ContainsKey PROP_SOURCE_CONTEXT) then
                    let initialStack = Runtime.getStackTraceIo ()
                    let stack =
                        initialStack.SkipWhile(fun x ->
                            not (x.Contains "Serilog.Log.Write") && not (x.Contains "Serilog.Core.Logger.Write")
                        ).ToList ()
                    if stack.Count > 0 then
                        let className =
                            stack.Select(fun x -> Regexxer.matchFirst(x, @"at ([\w\.`]+[^.])\..*?\("))
                                 .FirstOrDefault (fun (x: string) -> x.Contains "Serilog" |> not)
                        logEvent.AddOrUpdateProperty (propertyFactory.CreateProperty (PROP_SOURCE_CONTEXT, sprintf "%s*" className))
    ...
    let addLoggingSink (sinkConfig: LoggerConfiguration -> LoggerConfiguration) verbose =
        let loggerConfiguration = sinkConfig(LoggerConfiguration ())
        ...
        DelegateLogEventSink.AddSink (loggerConfiguration.MinimumLevel.Verbose()
                                        .Filter.With(LogFilter(filters, excludingFilters))
                                        .CreateLogger ())

        if Log.Logger = null || Log.Logger.GetType () <> typeof<DelegateLogEventSink> then
